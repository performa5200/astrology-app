...
def _worker_fetch_gemini(key, key_masked, prompt_text):
    ...
    for model_name in active_models:
        ...
        try:
            print(f"[{datetime.now().strftime('%H:%M:%S')}] 正在以 [{key_masked}] 請求 Gemini ({model_name})...")
            ...
            print(f"[{datetime.now().strftime('%H:%M:%S')}] ⚠️ Gemini ({model_name}) 未成功: {last_err}")
...
def call_gemini_api(prompt_text, keys):
    ...
    with ThreadPoolExecutor(max_workers=min(len(unique_keys), 8)) as executor:
        ...
        for future in as_completed(futures):
            ...
            if success:
                print(f"[{datetime.now().strftime('%H:%M:%S')}] 🏆 Gemini 金鑰 [{key_id}] 率先成功回傳 (模型: {model})")
                return res
...
with st.container():
    ...
    with c1:
        input_gender = st.selectbox("性別", ["坤造 (女)", "乾造 (男)"])
    with c2:
        input_date = st.date_input("出生公曆日期", value=date(2000, 1, 1), min_value=date(1900, 1, 1), max_value=date(2100, 12, 31))
    with c3:
        input_time = st.time_input("出生時間 (24h)", value=time(0, 0))

    gc1, gc2 = st.columns([1.5, 1])
    with gc1:
        input_location = st.text_input("出生國家 / 城市 (可中英文自由輸入，自動校正上升星座)", value="")
    with gc2:
...
with col_btn3:
    ...
    if report_ready:
        ...
        st.button("📂 輸出完整解析報告 (含AI解析)", disabled=True, use_container_width=True, help="請先點擊上方「開始 Gemini 專家深度詳算」生成報告")
...
st.markdown("""
💡 **操作備註：**
1. 右側「輸出完整解析報告」需在點擊左方「開始 Gemini 專家深度詳算」成功生成後才會啟用下載。
