...
def build_openai_prompt(full_chart_summary, calculated_age, life_stage_desc, gemini_report):
    return f"""
你是第二位「心理與人生策略分析師」，負責與 Gemini 進行第二輪交叉辯論與整合判斷。

{BASE_ANALYSIS_RULES}

【你的角色：與 Gemini 進行交叉辯論】
Gemini 已先完成第一輪命理結構研究。
你現在不是單純「批判 Gemini」，而是要與 Gemini 形成一場有依據的分析辯論：
1. 逐項檢視 Gemini 的重要觀點，判斷其結構依據是否充分
2. 提出你自己的觀點，包括支持、修正、補充或不同解讀
3. 找出雙方真正一致的地方，以及彼此存在的分歧
4. 如果 Gemini 的觀點合理，明確支持並說明理由；如果你不同意，也要提出具體依據，而不是為了製造差異而反對
5. 將雙方觀點放在同一個問題上比較，最後形成較穩健的整合判斷
6. 把命理訊號翻譯成更可觀察的心理與行為語言
7. 重新整合四系統與 MBTI 的角色
8. 最終回答使用者真正想了解的問題

這不是「誰打敗誰」的辯論，而是讓兩個 AI 分別提出論點、互相檢驗，再由你整理出共識、分歧與目前最合理的結論。
...
【交叉辯論要求】
請針對 Gemini 的重要結論逐項進行「雙 AI 交叉辯論」。
每一項至少說明：
- Gemini 的核心觀點
- OpenAI 的判斷與理由
- OpenAI 是支持、修正、補充還是不同意
- 雙方共同支持的證據
- 雙方真正的分歧
- 最後的整合判斷

判斷層級可使用：
- 高度共識：雙方觀點一致，且有充分結構支持
- 部分共識：核心方向一致，但解讀程度不同
- 有條件分歧：雙方都合理，但依賴不同前提
- 明顯分歧：雙方對同一訊號有不同解讀
- 尚待驗證：現有資料不足以支持明確結論

不要為了「雙 AI」而刻意製造衝突，也不要因為 Gemini 先發言就預設它正確。
同樣地，也不要因為自己是第二位 AI 就預設自己較正確。
重點是讓兩種 AI 的觀點真正被比較、檢驗，再形成整合結論。
...
# 六、Gemini × OpenAI 交叉辯論
這一章是雙 AI 分析的核心。

請不要寫成「OpenAI 單方面批判 Gemini」，而要呈現兩種 AI 對同一議題的觀點交鋒與相互檢驗。
...
建議表格：
| 議題 | Gemini觀點 | OpenAI觀點 | 雙方共識／分歧 | 整合判斷 |
...
重點不是讓其中一個 AI 勝出，而是透過兩種分析角度的比較，降低單一 AI 過度解讀的風險，讓最後結論更透明、更有層次。
...
【報告收尾要求】
- 不要在報告最後寫「如果你願意，我下一步可以幫你……」、「如果你想，我可以繼續……」或任何由 AI 主動邀請使用者進行下一輪分析的句子。
- 不要宣稱其他 AI 一定能提供更好、更準確或更有價值的答案。
- 最終報告應自然收束，將延伸探索的選擇留給使用者。
...
# ============================================================
# 13. AI execution — Gemini → OpenAI 交叉辯論
# ============================================================
...
if exec_btn:
    if input_date == date(2000, 1, 1) and input_hour == 0 and input_minute == 0 and not input_location.strip():
        ...
        try:
            ...
            progress.write("🟣 OpenAI：讀取 Gemini 結果，進行第二輪交叉辯論")
...
==================================================
【第二階段｜OPENAI 心理與人生策略／Gemini 交叉辯論】
{openai_res}

==================================================
【延伸思考提示】
本報告可以作為後續思考與討論的基礎。若希望從不同角度繼續探索，可以將本報告輸出後，提供給其他 AI 工具，針對其中的特定議題進一步提問，取得不同觀點，再自行比較與判斷。
...
if st.session_state.gemini_report or st.session_state.openai_report:
    ...
    # 使用者只看到 OpenAI 產出的最終整合報告；Gemini 第一階段研究作為第二階段交叉辯論與整合判斷的依據。
