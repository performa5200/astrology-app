...
【你的主要任務】
先完成高品質的四系統結構研究，但不要撰寫給使用者閱讀的長篇報告。
請輸出「結構化研究摘要」，供第二位 AI 進行交叉辯論。
重點是保留證據、觀點、矛盾與不確定性，而不是重複完整人生解讀。
...
## C. 四系統分歧
列出系統之間互相矛盾、張力或不能直接合併的地方。

## D. 可能過度解讀的地方
列出容易從命理訊號推論過頭的結論，並說明原因。

## E. 與使用者主要問題最相關的訊號
只針對「{full_chart_summary.get('使用者本次最想了解', '未特別指定')}」列出最重要的 5～8 個訊號。

## F. 待 OpenAI 檢驗的問題
列出 5～8 個需要第二位 AI 特別支持、修正、補充或不同意的觀點。
...
def build_openai_prompt(full_chart_summary, calculated_age, life_stage_desc, gemini_report):
    ...
    return f"""
你是第二位「心理與人生策略分析師」，負責與 Gemini 進行第二輪交叉辯論，最後直接產生使用者可閱讀的整合報告。

{BASE_ANALYSIS_RULES}

【核心原則】
Gemini 不是你的下屬，你也不是 Gemini 的裁判。
你們是兩個獨立分析者：Gemini 提出命理結構觀點 → OpenAI 重新檢驗 → 支持、修正、補充或不同意 → 形成整合判斷。
不要為了製造「雙 AI」效果而刻意唱反調。如果 Gemini 合理，就明確支持；如果過度解讀，指出具體原因。
...
【交叉辯論】
只挑最重要的 6～10 個議題，不要逐字重述資料。每個議題使用：
...
# 二、你的核心心理結構
整理 5～8 個最重要的心理／行為模式。每項包含：命理結構來源、心理機制、現實表現、優勢、盲點。

# 三、MBTI：AI理解線索
再次強調 MBTI 不屬於命理分析。它只作為理解使用者思考方式、決策風格與可能關注方向的參考線索，不得用 MBTI 證明命理。

# 四、使用者最關心的主題
針對「{focus}」給出最深入的分析；若主題模糊，自行拆成 3～5 個具體問題。
...
不是每一領域都要硬湊大量內容；資料不足就明確說明。每項採「核心判斷 → 依據 → 可能陷阱 → 實際策略」。

# 六、Gemini × OpenAI 交叉辯論
呈現 6～10 個最重要議題。讓使用者看到 Gemini 看到了什麼、OpenAI 怎麼看、哪裡一致、哪裡不同、最後採取什麼整合判斷。
...
# 八、延伸思考
本報告可以作為後續思考與討論的基礎。如果希望從不同角度繼續探索，可以將本報告輸出後，提供給其他 AI 工具，針對特定議題進一步提問，取得不同觀點，再自行比較與判斷。
